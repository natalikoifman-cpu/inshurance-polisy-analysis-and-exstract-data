...
fragments ("asked about billing on Tuesday", "asked again on Thursday").
Unmaintained memory grows stale, contradictory, and slow. Systems that only
ever ADD get worse with time; maintained memory gets **better** with time.
...
Typical measured effect: ~50 episodic conversation items compress to ~3 stable
facts (16.7×) — and retrieval quality goes UP because the noise is gone.

## With a backend action

Call a **Consolidate memories** action with a user's memory list. It returns
the compressed list plus what was merged/deleted and the compression ratio.
Schedule it with your platform's scheduler (cron, recurrence flow, scheduled
function) over users active that day.
...
- When a user contradicts a stored fact ("I don't live in Boston
  anymore"), confirm once, then treat the NEW fact as truth — the old
  one must be deleted, not kept alongside.
...
contradict each other on the user's city. It passes only if the output has one
